...
import { KATAKANA_GROUPS } from '../constants/katakana'
...
export default function KatakanaPage() {
  ...
  const handleNext = () => {
    ...
    } else if (selectedGroup < KATAKANA_GROUPS.length - 1) {
      setSelectedGroup(selectedGroup + 1)
      setCurrentIndex(0)
    }
    
    // 진행률 업데이트
    const totalChars = KATAKANA_GROUPS.reduce((acc, group) => acc + group.characters.length, 0)
    const completedChars = selectedGroup * 5 + currentIndex + 1
    const progress = (completedChars / totalChars) * 100
    updateProgress('katakana', progress)
  }
  ...
  const handlePrev = () => {
    if (currentIndex > 0) {
      ...
      setSelectedGroup(selectedGroup - 1)
      setCurrentIndex(KATAKANA_GROUPS[selectedGroup - 1].characters.length - 1)
    }
  }
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={handleNext}
          disabled={
            selectedGroup === KATAKANA_GROUPS.length - 1 &&
